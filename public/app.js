...
function renderResponses() {
  ...
  container.innerHTML = getActiveModels().map(m => {
    ...
    let body = '';
    if (status === 'loading') body = `<span style="color:#555;font-size:16px;letter-spacing:0.05em">thinking…</span>`;
    else if (status === 'done' && text) body = renderMarkdown(text);
    else if (status === 'error') body = `<span style="color:#663333;font-size:16px">failed — check key</span>`;
    return `
      <div style="display:flex;gap:12px;margin-bottom:16px;align-items:flex-start">
        <div style="min-width:62px;display:flex;align-items:center;gap:5px;padding-top:2px;flex-shrink:0">
          ...
          <span style="font-size:15px;color:${m.color};letter-spacing:0.04em;font-family:'DM Sans',sans-serif">${m.name}</span>
        </div>
        <div style="font-size:19px;color:#666;line-height:1.65;border-left:0.5px solid #1e1e1e;padding-left:12px;flex:1;font-family:'DM Sans',sans-serif">${body}</div>
      </div>`;
  }).join('');
}
...
function updateConsensus() {
  ...
  if (state.phase === 'synthesizing') {
    box.innerHTML = `<span style="font-size:16px;color:#555;letter-spacing:0.12em;font-family:'DM Sans',sans-serif">Synthesising…</span>`;
  } else if (state.synthesis) {
    box.innerHTML = `<div style="font-size:19px;color:#aaa;line-height:1.75;font-family:'DM Sans',sans-serif">${renderMarkdown(state.synthesis)}</div>`;
  } else if (state.error) {
    box.innerHTML = `<span style="color:#663333;font-size:18px;font-family:'DM Sans',sans-serif">${state.error}</span>`;
  }
}
...
function showResults() {
  ...
  document.getElementById('consensusBox').innerHTML = `<span style="font-size:16px;color:#555;letter-spacing:0.12em;font-family:'DM Sans',sans-serif">Querying models…</span>`;
